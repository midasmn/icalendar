...
$get_href = "http://www.oricon.co.jp/rank/js/w/";
$get_href .= $yyyy."-".$mm."-".$dd."/";
// $get_url = "https://
echo $get_href;
...
// var_dump($html);
//キーワード取得
foreach ($html->find('.rankitem_box_inr .item_img div a img') as $element)
{
    //ランク取得
    $rtn['img'][$cnt] = $element->src; 
    echo "<br>".$rtn['img'][$cnt];
    // $rtn['title'][$cnt] = $element->h2->plaintext; 
    // echo "<br>".$rtn['title'][$cnt];
    // $rtn['artist'][$cnt] = $element->h3->plaintext; 
    // echo "<br>".$rtn['artist'][$cnt];
